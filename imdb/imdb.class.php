<?php
 ...
 #----------------------------------------------------------[ Movie Rating ]---
  ...
  //echo '<table class=main2 border=0 cellspacing=0 cellpadding=10><tr><td>'.highlight_string($this->page["Title"], true).'<td></tr></table>';
    if ($this->page["Title"] == "") $this->openpage ("Title");
	if(preg_match("/\<span style=\"display:none\" id=\"star-bar-user-rate\"><b\>(.*?)\<\/b\>/m",$this->page["Title"],$match)){
      $this->main_rating = $match[1];
	}
	if(preg_match("/\<a    onclick\=\"(.*?)\"     href=\"ratings\"       >(.*?) votes<\/a\>/ms",$this->page["Title"],$match)){
 ...
 #---------------------------------------------------------------[ Tagline ]---
  ...
  function tagline () {
    if ($this->main_tagline == "") {
      if ($this->page["Title"] == "") $this->openpage ("Title");
      if (@preg_match("/\<h4 class=\"inline\">Taglines:<\/h4> \n(.*?)\n<span class=\"see-more inline\">/",$this->page["Title"],$match)) {
 ...
 #--------------------------------------------------------[ Photo specific ]---
  ...
  function photo () {
    if (empty($this->main_photo)) {
      if ($this->page["Title"] == "") $this->openpage ("Title");
      preg_match("/\<td rowspan=\"2\" id=\"img_primary\"\>\n(.*?)\>(.*?)<img src\=\"(.*?)\"/ms",$this->page["Title"],$match);
 ...
 #-----------------------------------------------------[ /plotsummary page ]---
  ...
  function plot () {
   if (empty($this->plot_plot)) {
    ...
    if (preg_match_all("/\<h2>Storyline<\/h2>(.*?)<\/p>/",str_replace("\n"," ",$this->page["Title"]),$matches))
      for ($i=1;$i<count($matches[1]);++$i)
	  print_r($matches[$i]);
        $this->plot_plot[$i] = $matches[$i];
   }
