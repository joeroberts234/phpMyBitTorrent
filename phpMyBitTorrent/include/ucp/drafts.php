...
*/
	...
	"do"			=> "edit_take"
		...
		$link_forum = true;
		$sql_f = "SELECT name FROM `".$db_prefix."_forum_forums` WHERE `id` = '".$row['forum_id']."'";
		$res_f = $db->sql_query($sql_f) or btsqlerror($sql_f);
		$row_f = $db->sql_fetchrow($res_f);
		$tittle = $row_f['name'];
		$insert_url = 'forums.php?action=newtopic&forumid=' . $row['forum_id'] . '&d='.$row['draft_id'];
		$view_url = 'user.php?op=editprofile&amp;action=overview&mode=drafts&edit=' . $row['draft_id'];
		}
$template_row = array(
'DATE' =>format_date2($row['save_time']),
