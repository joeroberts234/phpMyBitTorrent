...
$res2 = mysql_query("SELECT uid, seeder, torrent, client FROM ".$db_prefix."_peers WHERE connectable='no' ORDER BY uid DESC") or sqlerr();
...
$result = mysql_query("select distinct uid from ".$db_prefix."_peers where connectable = 'no'");
$count = mysql_num_rows($result);
print ("$count unique users that are not connectable.");
@mysql_free_result($result);

if (mysql_num_rows($res2) == 0)
print("<p align=center><b>All Peers Are Connectable! :D</b></p>\n");
else
{
...
{
$r2 = mysql_query("SELECT username FROM ".$db_prefix."_users WHERE id=$arr2[uid]") or sqlerr();
$a2 = mysql_fetch_assoc($r2);
print("<tr><td><a href=userdetails.php?id=$arr2[uid]>$a2[username]</a></td><td align=left><a href=details.php?id=$arr2[torrent]&dllist=1#seeders>$arr2[torrent]");
if ($arr2[seeder] == 'yes')
...
$query = mysql_query("SELECT distinct uid FROM ".$db_prefix."_peers WHERE connectable='no'");
while($dat=mysql_fetch_assoc($query)){
mysql_query("INSERT INTO messages (sender, receiver, added, msg) VALUES (0,$dat[uid] , '" . get_date_time() . "', " . sqlesc($msg) .")") or sqlerr(__FILE__,__LINE__);
}
...
$r2 = mysql_query("SELECT username FROM ".$db_prefix."_users WHERE id=$arr2[user]") or sqlerr();
$a2 = mysql_fetch_assoc($r2);
