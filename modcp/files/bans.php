...
if (!eregi("modcp.php",$_SERVER["PHP_SELF"])) die ("You can't access this file directly");
        ...
        case "addban": {
                if (isset($postback_ip)) { //Ban IP
                        ...
                        if (isset($returnto)) switch ($returnto) {
                                case "user": {
                                        if (isset($page)) header("Location: modcp.php?op=user&page=".$page);
                                        else header("Location: modcp.php?op=user");
                                                die();
                                }
                        }
                }
...
if ($db->sql_numrows($res) < 1) {
        ...
        while ($ban = $db->sql_fetchrow($res)) {
                echo "<tr><td><p>".@long2ip($ban["ipstart"])."</p></td><td><p>".@long2ip($ban["ipend"])."</p></td><td><p>".htmlspecialchars($ban["reason"])."</p></td><td><p>".pic("edit.gif","modcp.php?op=editban&id=".$ban["id"]).pic("drop.gif","modcp.php?op=delban&id=".$ban["id"])."</p></td></tr>\n";
        }
        echo "</tbody>\n";
        echo "</table>\n";
}
...
if ($db->sql_numrows($res) < 1) {
        ...
        echo "</thead>\n";
        while ($ban = $db->sql_fetchrow($res)) {
                echo "<tr><td><p><a href=\"user.php?op=profile&id=".$ban["id"]."\">".$ban["username"]."</a></p></td><td><p>".htmlspecialchars($ban["banreason"])."</p></td><td><p>".pic("drop.gif","modcp.php?op=delban&uid=".$ban["id"])."</p></td></tr>\n";
        }
        echo "</table>\n";
}
$db->sql_freeresult($res);
