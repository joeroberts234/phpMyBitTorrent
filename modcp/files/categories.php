...
function RebuildSortIndex() {
        global $db, $db_prefix;
        $sql = "SELECT id FROM ".$db_prefix."_categories ORDER BY sort_index ASC;";
...
switch($op) {
        case "addcategory": {
                ...
                $sort++;
                $sql = "INSERT INTO ".$db_prefix."_categories (name, sort_index, image) VALUES ('".addslashes($sub_name)."', '".$sort."', '".addslashes(html_entity_decode($sub_image))."');";
                $db->sql_query($sql) or btsqlerror($sql);
                $op = "";
                RebuildSortIndex();
                break;
        ...
        case "editcategory": {
                ...
                if (isset($sub_name) AND isset($sub_image)) {
                        $sql = "UPDATE ".$db_prefix."_categories SET name = '".addslashes($sub_name)."', image = '".addslashes(html_entity_decode($sub_image))."' WHERE id = '".$id."';";
                        $db->sql_query($sql) or btsqlerror($sql);
                        $op = "";
                }
                break;
...
if ($db->sql_numrows($res) < 1) {
        ...
        while ($row = $db->sql_fetchrow($res)) {
                echo "<tr>";
                echo "<td><p align=\"center\">".pic("edit.gif","modcp.php?op=editcategory&id=".$row["id"])."\n".pic("drop.gif","modcp.php?op=delcategory&id=".$row["id"])."</p></td>\n";
...
echo "<form method=\"POST\" action=\"modcp.php\">\n";
...
echo "<p>&nbsp</p>\n";
echo "<form method=\"POST\" action=\"modcp.php?op=categories\"enctype=\"multipart/form-data\">\n\n";
