...
$totsql = "SELECT COUNT(id) FROM ".$db_prefix."_private_messages WHERE recipient = '".$user->id."';";
...
if($page < $pages) {
        $next = "<b><a href=\"pm.php?op=inbox&page=".($page+1)."\">>>>></a></b>";
} else {
        $next = ">>>>";
}
if ($page > 1) {
        $prev = "<b><a href=\"pm.php?op=inbox&page=".($page-1)."\"><<<<</a></b>";
} else {
        $prev = "<<<<";
}
$pager = "<a href=\"pm.php?op=inbox\">".(($page == 1) ? "<b>1</b>" : "1")."</a>&nbsp;";

if (($page - 15) > 1) $pager .= "...";

for ($i = max(2,$page - 15); $i < min($pages, $page + 15); $i++) {
        $pager .= "<a href=\"pm.php?op=inbox&page=".$i."\">".(($i == $page) ? "<b>".$i."</b>" : $i)."</a>&nbsp;";
}
if (($page + 15) < $pages) $pager .= "...";
$pager .= "<a href=\"pm.php?op=inbox&page=".$pages."\">".(($page == $pages) ? "<b>".$pages."</b>" : $pages)."</a>";

OpenTable(_btinbox);

echo "<form name=\"folder\" action=\"pm.php\" method=\"POST\">\n";
                    echo "<select name=\"op\" onchange=\"javascript: document.folder.submit();\"> <option value=\"inbox\" selected>Inbox</option><option value=\"outbox\">Outbox</option></select>\n";
                        echo "</form><br>\n";

$sql = "SELECT P.*, U.username AS username, IF (U.name IS NULL, U.username, U.name) as user_name, U.level as user_level FROM ".$db_prefix."_private_messages P LEFT JOIN ".$db_prefix."_users U ON P.sender = U.id WHERE P.recipient = '".$user->id."' AND recipient_del = 'false' ORDER BY P.sent DESC LIMIT ".$from.",20;";
...
if (!$db->sql_numrows($res)) {
        ...
        echo "<form method=\"POST\" name=\"privmsg_inbox\" action=\"pm.php\">\n";
        echo "<input type=\"hidden\" name=\"op\" value=\"inbox_delete\" />\n";
        echo "<table style=\"border-collapse:collapse;\" border=\"1\" bordercolor=\"#CDCDCF\" cellpadding=\"3\" cellspacing=\"0\" width=\"100%\" >\n";
        echo "<thead align=\"center\">\n<tr>\n";
        echo "<th width=\"2%\" nowrap><p>&nbsp;</p></th>\n";
        echo "<th nowrap><p>"._btpmsub."</p></th>\n";
        echo "<th nowrap><p>"._btpmfrom."</p></th>\n";
        echo "<th nowrap><p>"._btpmdate."</p></th>\n";
        ...
        while ($msg = $db->sql_fetchrow($res)) {
                ...
                echo "</p></td>\n";
                echo "<td><p align=\"center\"><a href=\"pm.php?op=readmsg&mode=inbox&mid=".$msg["id"]."\">".stripslashes($msg["subject"])."</a></p></td>\n";
		        ...
		        if($msg["sender"] == 0)echo "<td><p align=\"center\">System</p></td>\n";
				if($msg["sender"] !=0)echo "<td><p align=\"center\"><a class=\"userclass\" href=\"user.php?op=profile&id=".$msg["sender"]."\">".$msg["user_name"]."</a></p></td>\n";
        ...
        echo pic("pm_delete.png","javascript:document.forms['privmsg_inbox'].submit()",_btpmdelete).pic("pm_delall.png","pm.php?op=inbox_delall",_btpmdelall);
        echo "</td>\n";
        echo "</tr>\n";
        echo "</table>\n";
...
echo "<tr><td width=\"50%\">";
//Friends
echo "<table width=\"100%\" border=\"1\" bordercolor=\"#CDCDCF\" cellpadding=\"3\" cellspacing=\"0\">\n";
