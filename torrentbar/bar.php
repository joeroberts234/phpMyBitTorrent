...
// Database Presets
$config_path = "../include/configdata.php";
...
if ($userid!="") {
	...
	$query = "SELECT count(id) FROM ".$db_prefix."_users WHERE id = '".$userid."'";
    $result = @mysql_query($query) or die("Could not select data!");
    $counter = mysql_result($result, 0);
    mysql_free_result($result);
    
    if ($counter>0) {
    	$query = "SELECT uploaded, downloaded FROM ".$db_prefix."_users WHERE id = ".$userid;
        $result = mysql_query($query) or die("Could not select data!");
