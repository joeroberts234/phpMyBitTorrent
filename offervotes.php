...
$count = $row[0];
$perpage = 5;
$res = $db->sql_query("select ".$db_prefix."_users.id as userid,".$db_prefix."_users.username, ".$db_prefix."_users.downloaded,".$db_prefix."_users.uploaded,".$db_prefix."_users.regdate, ".$db_prefix."_offers.id as offerid, ".$db_prefix."_offers.name from ".$db_prefix."_offervotes inner join ".$db_prefix."_users on ".$db_prefix."_offervotes.userid = ".$db_prefix."_users.id inner join ".$db_prefix."_offers on ".$db_prefix."_offervotes.offerid = ".$db_prefix."_offers.id WHERE ".$db_prefix."_offervotes.offerid =$offerid $limit");
$res2 = $db->sql_query("select name from ".$db_prefix."_offers where id=$offerid");
$arr2 = $db->sql_fetchrow($res2);
OpenTable("Votes for ".$arr2[name]);
...
"<td class=colhead align=left>Ratio</td><td class=colhead align=left>Joined</td>\n");
...
$joindate = "$arr[regdate] (" . get_elapsed_time(sql_timestamp_to_unix_timestamp($arr["regdate"])) . " ago)";
$downloaded = mksize($arr["downloaded"]);
if ($arr["active"] == 0)
$enabled = "<font color = red>Non</font>";
else
$enabled = "<font color = green>Oui</font>";
print("<tr><td><a href=user.php?op=profile&id=$arr[userid]><b>$arr[username]</b></a></td><td align=left>$uploaded</td><td align=left>$downloaded</td><td align=left>$ratio</td><td align=left>$joindate</td></tr>\n");
}
print("</table></div>\n");
