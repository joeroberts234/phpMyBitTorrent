...
echo "</td></tr><tr><td><a href=\"admin.php?op=user&dupeaccounts=1\">Search for duplicate IP's</a></td></tr></table>";
...
}elseif (!empty($dupeaccounts)) {
        $sql = "SELECT id, username, email, level, ban, uploaded, downloaded, regdate, lastip, lasthost, lastlogin FROM ".$db_prefix."_users GROUP BY lastip HAVING ( COUNT(lastip) > 1 );";
        $tot = 1;
        $res = $db->sql_query($sql);
...
while ($row = $db->sql_fetchrow($res)) {
	...
	echo "<td style=\"text-align: center; border-bottom:0; border-left:0; border-right:0;\"><p>";
        $info .=  "<br />".pic("pic_ratio.gif").$ratio;
        help(pic("help.gif"),$info,_btinfo);
       echo _admuserlastip.": ".long2ip($row["lastip"])." <br>[".$row["lasthost"]."]";
        echo"</p></td>";
        echo "</tr>\n";
        ...
        if (!$row["ban"]) echo adminpic("user_ban.png","admin.php?op=addban&username=".urlencode($row["username"])."&returnto=user&page=".$page,_admuserban);
        else echo adminpic("user_unban.png","admin.php?op=delban&uid=".$row["id"]."&returnto=user&page=".$page,_admuserunban);
        echo "</p></td></tr>\n";
...
$pager = "<a href=\"admin.php?op=user\">".(($page == 1) ? "<b>1</b>" : "1")."</a>&nbsp;";

if (($page - 15) > 1) $pager .= "...";

for ($i = max(2,$page - 15); $i < min($pages, $page + 15); $i++) {
        $pager .= "<a href=\"admin.php?op=user&page=".$i."\">".(($i == $page) ? "<b>".$i."</b>" : $i)."</a>&nbsp;";
}

if (($page + 15) < $pages) $pager .= "...";
$pager .= "<a href=\"admin.php?op=user&page=".$pages."\">".(($page == $pages) ? "<b>".$pages."</b>" : $pages)."</a>";
